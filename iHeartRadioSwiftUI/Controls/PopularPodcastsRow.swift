...
struct PopularPodcastCell: View {
    var podcast: Podcast
    ...
    var body: some View {
        VStack {
            ...
            VStack(alignment: .leading) {
                Text(podcast.title).font(.subheadline)
                Text(podcast.subtitle).font(.caption)
            }
            .frame(width: 100)
        }
    }
...
struct PopularPodcastsRow : View {
            ...
            ScrollView(showsHorizontalIndicator: false) {
                HStack(alignment: .top, spacing: 0) {
                    ForEach(podcastList.podcasts) { podcast in
                        PopularPodcastCell(podcast: podcast)
                        Spacer()
                    }
                }
            }
...
#if DEBUG
struct PopularPodcastsRow_Previews : PreviewProvider {
    ...
    static var previews: some View {
        PopularPodcastsRow(podcastList: PodcastList(id: 1, name: "Popular", podcasts: samplePodcasts))
    }
}
#endif
